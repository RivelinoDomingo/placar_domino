      ...
      async (event) => {
        try {
          ...
          if (beforeData.series !== afterData.series) {
            ...
            const oldIndex =
              seriesOrder.indexOf(beforeData.series || "Amador");
            const newIndex =
              seriesOrder.indexOf(afterData.series || "Amador");
          ...
          if (title && body) {
            ...
            if (subscriptionsSnapshot.empty) {
              console.log("Nenhuma inscrição encontrada para" +
                "enviar notificações.");
              return null;
            }
            ...
            response.results.forEach((result, index) => {
              ...
              if (error) {
                console.error("Falha ao enviar para o token",
                    tokens[index], error);
                // Se o token não é mais válido, o removemos do banco de dados.
                if (error.code === "messaging/registration" +
            ...
            console.log("Envio concluído e limpeza de tokens" +
            " inválidos realizada.");
          }
          return null;
        } catch (error) {
